...
def create_line_chart(chat_id, df):
    ...
    plt.plot(df["Категория"], df["Сумма"], marker='o')
...
def work_with_sotrudnik(message):
    df = view_table(selected_role, 'sotrudnik')
    table_text = "```\n" + df.to_markdown(index=False) + "\n```"
    bot.send_message(message.chat.id, table_text, parse_mode="Markdown")

    keyboard = telebot.types.ReplyKeyboardMarkup(row_width=1, one_time_keyboard=True)
    buttons = ["Добавить запись", "Удалить запись", "Обновить запись"]
    keyboard.add(*[telebot.types.KeyboardButton(text=button) for button in buttons])
    bot.send_message(message.chat.id, "Выберите действие с таблицей 'sotrudnik':", reply_markup=keyboard)
    bot.register_next_step_handler(message, process_sotrudnik_action)

def process_sotrudnik_action(message):
    action = message.text
    if action == "Добавить запись":
        bot.send_message(message.chat.id, "Введите данные нового сотрудника в формате 'ФИО, ceh_id':")
        bot.register_next_step_handler(message, add_sotrudnik_record)
    elif action == "Удалить запись":
        bot.send_message(message.chat.id, "Введите фамилию сотрудника, которого хотите удалить:")
        bot.register_next_step_handler(message, delete_sotrudnik_record)
    elif action == "Обновить запись":
        bot.send_message(message.chat.id, "Введите id сотрудника, данные которого хотите обновить:")
        bot.register_next_step_handler(message, update_sotrudnik_record)
    else:
        bot.send_message(message.chat.id, "Некорректное действие")
        work_with_sotrudnik(message)

def add_sotrudnik_record(message):
    try:
        name, ceh_id = message.text.split(', ')
        query = "INSERT INTO sotrudnik (name, ceh_id) VALUES (%s, %s);"
        with database_connection(selected_role) as conn:
            cursor = conn.cursor()
            cursor.execute(query, (name, ceh_id))
        bot.send_message(message.chat.id, "Запись успешно добавлена")
    except Exception as e:
        bot.send_message(message.chat.id, f"Ошибка при добавлении записи: {e}")
    work_with_sotrudnik(message)

def delete_sotrudnik_record(message):
    try:
        surname = message.text
        query = "DELETE FROM sotrudnik WHERE name LIKE %s;"
        with database_connection(selected_role) as conn:
            cursor = conn.cursor()
            cursor.execute(query, (f"%{surname}%",))
        bot.send_message(message.chat.id, "Запись успешно удалена")
    except Exception as e:
        bot.send_message(message.chat.id, f"Ошибка при удалении записи: {e}")
    work_with_sotrudnik(message)

def update_sotrudnik_record(message):
    try:
        sotrudnik_id = message.text
        bot.send_message(message.chat.id, "Введите новые данные сотрудника в формате 'ФИО, ceh_id':")
        bot.register_next_step_handler(message, lambda m: process_update_sotrudnik_record(m, sotrudnik_id))
    except Exception as e:
        bot.send_message(message.chat.id, f"Ошибка при обновлении записи: {e}")
    work_with_sotrudnik(message)

def process_update_sotrudnik_record(message, sotrudnik_id):
    try:
        name, ceh_id = message.text.split(', ')
        query = "UPDATE sotrudnik SET name = %s, ceh_id = %s WHERE id = %s;"
        with database_connection(selected_role) as conn:
            cursor = conn.cursor()
            cursor.execute(query, (name, ceh_id, sotrudnik_id))
        bot.send_message(message.chat.id, "Запись успешно обновлена")
    except Exception as e:
        bot.send_message(message.chat.id, f"Ошибка при обновлении записи: {e}")
    return work_with_sotrudnik(message)
